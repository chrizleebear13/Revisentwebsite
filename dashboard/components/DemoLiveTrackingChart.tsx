...
export function DemoLiveTrackingChart() {
  ...
  const { chartData, stats, topItems } = useMemo(() => {
    ...
    if (timeFrame === 'D') {
      ...
      if (!appliedStartDate || !appliedEndDate) {
        return { chartData: [], stats: { total: '0', trash: '0', recycle: '0', compost: '0' }, topItems: [] }
      }
    ...
    const filteredDetections = DEMO_DETECTIONS.filter(d => {
    ...
    filteredDetections.forEach((item) => {
      ...
      if (slotIdx >= 0 && slotIdx < grouped.length) {
        ...
        group.total++
        const category = item.category as 'compost' | 'recycle' | 'trash'
    ...
    // Calculate totals
    const totals = filteredDetections.reduce(
      (acc, item) => {
        acc.total++
        const category = item.category as 'compost' | 'recycle' | 'trash'
        if (category in acc) {
    ...
    filteredDetections.forEach((item) => {
      ...
      if (existing) {
        ...
        itemCounts.set(name, { count: 1, category: item.category })
      }
    })
    ...
    const sortedItems = Array.from(itemCounts.entries())
      .map(([name, data]) => ({
        ...
        percentage: filteredDetections.length > 0 ? (data.count / filteredDetections.length) * 100 : 0
      }))
  ...
  }, [timeFrame, appliedStartDate, appliedEndDate])
